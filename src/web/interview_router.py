...
def create_interview_endpoint(interview: schemas.InterviewCreate, database: Session = Depends(get_db)):
  return interview_service.create_interview(database, interview)


@router.get("/interviews/", response_model=List[schemas.Interview])
def read_interviews_endpoint(skip: int = 0, limit: int = 100, database: Session = Depends(get_db)):
...
def read_interview_endpoint(interview_id: int, database: Session = Depends(get_db)):
...
def update_interview_endpoint(interview_id: int, interview: schemas.InterviewCreate, database: Session = Depends(get_db)):
...
def delete_interview_endpoint(interview_id: int, database: Session = Depends(get_db)):
